...
SCRIPT = Path(__file__).parent / "vps-configure-luker-tts.sh"
...
def main():
    ...
    sftp = c.open_sftp()
    remote = "/tmp/vps-configure-luker-tts.sh"
    with sftp.file(remote, "w") as f:
        f.write(SCRIPT.read_text(encoding="utf-8"))
    sftp.chmod(remote, 0o755)
    sftp.close()
    _, o, e = c.exec_command(f"bash {remote}", get_pty=True, timeout=600)
